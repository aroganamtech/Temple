...
const CSS = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  /* ── CARDS ── */
  .temple-card{background:#FFFFFF;border:1px solid #E2D5C0;overflow:hidden;
    transition:all .4s cubic-bezier(.23,1,.32,1);display:flex;flex-direction:column;
    cursor:pointer;border-radius:10px;position:relative;
    box-shadow:0 2px 10px rgba(0,0,0,.05)}
  .temple-card:hover{border-color:#D4AF37;
    box-shadow:0 14px 44px rgba(212,175,55,.18),0 2px 8px rgba(0,0,0,.08);
    transform:translateY(-8px)}
  .temple-card:hover .card-img{transform:scale(1.08);filter:brightness(1.05) saturate(1.2)}
  .temple-card:hover .c-book{opacity:1;transform:translateY(0)}

  .card-img-wrap{position:relative;height:222px;overflow:hidden;background:#eee;flex-shrink:0}
  ...
  .card-type-chip{position:absolute;top:12px;left:12px;z-index:5;padding:4px 11px;
    background:rgba(26,21,16,.82);border:1px solid #D4AF3750;color:#D4AF37;
    font-size:10px;font-weight:600;letter-spacing:.14em;text-transform:uppercase;
    font-family:'Poppins',sans-serif;border-radius:3px;backdrop-filter:blur(6px)}
  .card-days-chip{position:absolute;top:12px;right:12px;z-index:5;padding:4px 11px;
    background:#D4AF37;color:#1A1208;font-size:10px;font-weight:700;
    letter-spacing:.1em;font-family:'Poppins',sans-serif;border-radius:3px}
  .card-name-block{position:absolute;bottom:0;left:0;right:0;padding:18px 16px 12px}
  .card-content{padding:16px 18px 18px;display:flex;flex-direction:column;flex:1;gap:10px}
  .card-deity-row{display:flex;align-items:center;gap:7px;padding-bottom:10px;border-bottom:1px solid #F0E8D8}
  .card-price-row{display:flex;justify-content:space-between;align-items:flex-end;margin-top:auto}
  .c-book{opacity:0;transform:translateY(6px);transition:all .35s cubic-bezier(.23,1,.32,1);margin-top:10px}
  .card-shine{position:absolute;top:0;left:-100%;width:60%;height:100%;
    background:linear-gradient(90deg,transparent,rgba(255,255,255,.06),transparent);
    pointer-events:none;z-index:10}
  @keyframes shine{from{left:-100%}to{left:200%}}
  .temple-card:hover .card-shine{animation:shine .65s ease forwards}
